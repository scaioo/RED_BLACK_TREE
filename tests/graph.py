...
# load data from CSV
df = pd.read_csv('../build/rbt_ratio_stats.csv')

plt.figure(figsize=(10, 6))

# Plot Ratio Insert with Error Bars (Sigma)
plt.errorbar(df['N'], df['RatioInsert_BlockMean'], yerr=df['RatioInsert_BlockSigma'], 
             fmt='-o', capsize=5, label='Ratio Insert (Mean ± σ)', color='blue')

# Plot Ratio Delete with Error Bars (Sigma)
plt.errorbar(df['N'], df['RatioDelete_BlockMean'], yerr=df['RatioDelete_BlockSigma'], 
             fmt='-s', capsize=5, label='Ratio Delete (Mean ± σ)', color='red')

plt.xscale('log') 
plt.xlabel('Input Size (N)')
plt.ylabel('Ratio coefficient (Time / log N)')
